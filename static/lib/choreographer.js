...
function zoomTo(elementId, seconds) { 
	tv.do(document.getElementById(elementId), seconds,document);
}  
...
//	delete pending[item.rule];
	...
	$(rule+" iframe").load(function () { 
		$('#debug').append("<li>iframe"+id+" loaded</li>");
		if(typeof $(this)[0].contentWindow.setName != 'undefined') { 
			$(this)[0].contentWindow.setName(byId[id].rule);
		} 
